...
/* AA::Needed for cygwin compiler. */	
#include <stdlib.h>


bool mem_interface::disambiguate(unsigned int lq_index,
...
void mem_interface::next_replay_index(bool restart) {
   unsigned int scan;
...
mem_interface::mem_interface(unsigned int lq_size, unsigned int sq_size, unsigned int Tid):
			DC(L1_DC_SETS,
			   L1_DC_ASSOC,
			   L1_DC_LINE_SIZE,
			   L1_DC_HIT_LATENCY,
...
bool mem_interface::stall(unsigned int bundle_load, unsigned int bundle_store) {
	return( ((lq_length + bundle_load)  > lq_size) ||
	        ((sq_length + bundle_store) > sq_size) );
}


void mem_interface::dispatch(bool load,
			     unsigned int size,
			     bool left,
			     bool right,
			     bool is_signed,
...
void mem_interface::store_addr(SS_TIME_TYPE cycle,
			       unsigned int addr,
...
void mem_interface::store_value(unsigned int sq_index, SS_WORD_TYPE value0, SS_WORD_TYPE value1) {
	assert(SQ[sq_index].valid);
...
bool mem_interface::load_addr(SS_TIME_TYPE cycle,
			      unsigned int addr,
...
bool mem_interface::load_replay(SS_TIME_TYPE cycle, unsigned int& pay_index, SS_WORD_TYPE& value0, SS_WORD_TYPE& value1) {
   bool unstalled;
...
void mem_interface::execute_load(SS_TIME_TYPE cycle,
				 unsigned int lq_index,
				 unsigned int sq_index, bool sq_index_phase) {
   bool stall_disambig;
   bool forward;
...
void mem_interface::checkpoint(unsigned int& chkpt_lq_tail, bool& chkpt_lq_tail_phase,
...
void mem_interface::restore(unsigned int recover_lq_tail, bool recover_lq_tail_phase,
...
void mem_interface::commit(bool load) {
	if (load) {
...
void mem_interface::copy_mem(char **master_mem_table) {
   for (unsigned int i = 0; i < MEMORY_TABLE_SIZE; i++) {
...
// STATS
void mem_interface::stats(FILE *fp) {
...
char *mem_interface::mem_newblock(void) {
